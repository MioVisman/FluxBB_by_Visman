...
if (isset($_GET['action']) || isset($_GET['search_id']))
{
	...
	{
		$keywords = isset($_GET['keywords']) ? mb_strtolower(pun_trim($_GET['keywords'])) : '';
		$author = isset($_GET['author']) ? mb_strtolower(pun_trim($_GET['author'])) : '';
...
#					$db->free_result($result);
		...
		else if ($action == 'show_new' || $action == 'show_recent' || $action == 'show_replies' || $action == 'show_user_posts' || $action == 'show_user_topics' || $action == 'show_subscriptions' || $action == 'show_unanswered')
		{
			...
			if ($action == 'show_new')
			{
				...
				$no_hits_mess = $lang_search['No new posts'];
			}
			...
			else if ($action == 'show_recent')
			{
				...
				$no_hits_mess = $lang_search['No recent posts'];
			}
			...
			else if ($action == 'show_replies')
			{
				...
				$no_hits_mess = $lang_search['No user posts'];
			}
			...
			else if ($action == 'show_user_topics')
			{
				...
				$no_hits_mess = $lang_search['No user topics'];
			...
			else if ($action == 'show_subscriptions')
			{
				...
				$no_hits_mess = $lang_search['No subscriptions'];
			...
			else
			{
				...
				$no_hits_mess = $lang_search['No unanswered'];
			}
...
//
	...
	if (!empty($search_ids))
	{
		...
		else if ($search_type[0] == 'warn') // MOD warnings - Visman
			$crumbs_text['search_type'] = '<a href="search.php?action=show_user_warn&amp;user_id='.$search_type[1].'">'.sprintf($lang_search['Quick search show_user_warn'], pun_htmlspecialchars($search_set[0]['pposter'])).'</a>';
		else
		{
			$keywords = $author = '';
...
<?php if (!empty($cur_search['wposter'])): ?>
					<div class="postwarn">
						<?php echo $warnings."\n" ?>
					</div>
<?php endif; ?>

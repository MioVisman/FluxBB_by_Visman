...
class addon_security_for_post extends flux_addon
{
	...
	function hook_post_after_validation()
	{
		...
		$result = $db->query('SELECT * FROM '.$db->prefix.'sec_of_post WHERE form_key=\''.$db->escape($form_key).'\' LIMIT 1') or error('Unable to get sec_of_post data', __FILE__, __LINE__, $db->error());
		...
		if (empty($errors))
			$db->query('DELETE FROM '.$db->prefix.'sec_of_post WHERE form_key=\''.$db->escape($form_key).'\'') or error('Unable to delete sec_of_post data', __FILE__, __LINE__, $db->error());
		else
			$db->query('UPDATE '.$db->prefix.'sec_of_post SET form_captcha=\'error\' WHERE form_key=\''.$db->escape($form_key).'\'') or error('Unable to update sec_of_post data', __FILE__, __LINE__, $db->error());
	}

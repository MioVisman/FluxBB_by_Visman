...
$link_for_title = 'userlist.php?username='.urlencode($username).'&amp;show_group='.$show_group.'&amp;'.(($p>1) ? 'p='.$p.'&amp;' : '').'sort_by=';
...
<div id="users1" class="blocktable">
	...
	<div class="box">
		<div class="inbox">
			...
			<thead>
				<tr>
					<th class="tcl" scope="col"><?php echo (($sort_by == 'username') ? (($sort_dir == 'DESC') ? '&darr; ' : '&uarr; ') : '').'<a href="'.$link_for_title.'username'.(($sort_by == 'username' && $sort_dir == 'ASC') ? '&amp;sort_dir=DESC' : '').'">'.$lang_common['Username'].'</a>' ?></th>
					<th class="tc2" scope="col"><?php echo $lang_common['Title'] ?></th>
<?php if ($show_post_count): ?>					<th class="tc3" scope="col"><?php echo (($sort_by == 'num_posts') ? (($sort_dir == 'DESC') ? '&darr; ' : '&uarr; ') : '').'<a href="'.$link_for_title.'num_posts'.(($sort_by == 'num_posts' && $sort_dir == 'ASC') ? '&amp;sort_dir=DESC' : '').'">'.$lang_common['Posts'].'</a>' ?></th>
<?php endif; ?>					<th class="tcr" scope="col"><?php echo (($sort_by == 'registered') ? (($sort_dir == 'DESC') ? '&darr; ' : '&uarr; ') : '').'<a href="'.$link_for_title.'registered'.(($sort_by == 'registered' && $sort_dir == 'ASC') ? '&amp;sort_dir=DESC' : '').'">'.$lang_common['Registered'].'</a>' ?></th>
				</tr>
			</thead>
			<tbody>
<?php

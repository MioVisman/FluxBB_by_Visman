...
// проверка IP админов и модераторов - Visman
	...
	$error = '';
	if (empty($_POST['csrf_token']) || empty($_POST['cr']) || empty($_POST['req_username']) || empty($_POST['req_password']) || empty($_POST['redirect_url']))
		$error = 'Error 1: '.$lang_sec['You are robot'];
			...
			<div class="inform">
				<fieldset>
					...
					<div class="infldset">
						...
						<input type="hidden" name="<?php echo random_for_crypto('redirect_url') ?>" value="<?php echo pun_htmlspecialchars($redirect_url) ?>" />
						<input type="hidden" name="<?php echo random_for_crypto('csrf_token') ?>" value="<?php echo csrf_hash() ?>" />
						...
						<div class="rbox clearb">
							<label><input type="checkbox" name="<?php echo random_for_crypto('save_pass') ?>" value="1" tabindex="3" /><?php echo $lang_login['Remember me'] ?><br /></label>
						</div>
